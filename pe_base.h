...
#pragma pack(pop)
	...
	virtual inline const IMAGE_DOS_HEADER& get_dos_header() const
	{
		return dos_header_;
	}

	virtual inline IMAGE_DOS_HEADER& get_dos_header()
	{
		return dos_header_;
	}

	virtual inline LONG get_pe_header_start() const
	{
		return dos_header_.e_lfanew;
	}

	virtual inline const std::string& get_stub_overlay() const
	{
		return rich_overlay_;
	}

	virtual inline std::vector<pe_base::section>& get_image_sections()
	{
		return sections_;
	}

	virtual inline const std::vector<pe_base::section>& get_image_sections() const
	{
		return sections_;
	}
	virtual ~pe_base()
